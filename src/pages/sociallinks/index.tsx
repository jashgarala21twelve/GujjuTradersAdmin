...
const SocialLinks = () => {
  ...
  const { mutate: useSocialLinkMutation, isPending: isSocialLinkCreating } =
    useSocialLink(onSuccessCreateCoupon);
  ...
  useEffect(() => {
    if (
      isSuccess &&
      apiResponse &&
      JSON.stringify(data) !== JSON.stringify(apiResponse)
    ) {
      ...
      <Form {...form}>
        ...
        >
          ...
          <div className='flex justify-left space-x-4'>
            <Button type='submit' disabled={isSocialLinkCreating}>
              {' '}
              {isSocialLinkCreating ? 'Submitting...' : 'Submit'}
            </Button>
          </div>
        </form>
      </Form>
